...
const findCommonPoints = (arrayA, arrayB) => {
  match = [];
  match = arrayA.filter(value => {
    return arrayB.includes(value);
  });
  return match.map(val => JSON.parse(val));
};

const manhattenDistance = (start = [0, 0], end) => {
  return Math.abs(end[0] - start[0]) + Math.abs(end[1] - start[1]);
};

// let wire_c1 = getCoordinates(wire1);
// let wire_c2 = getCoordinates(wire2);

// let commonPoints = findCommonPoints(
//   wire_c1.map(val => JSON.stringify(val)),
//   wire_c2.map(val => JSON.stringify(val))
// );

// console.log(commonPoints);

// console.log(commonPoints.map(val => manhattenDistance([0, 0], val)));


let commonPoints = [
  [ 0, 0 ],        [ -1932, 3049 ], [ -2093, 3105 ],
  [ -2723, 3584 ], [ -3370, 5494 ], [ -3370, 5598 ],
  [ -4173, 6207 ], [ -4343, 6380 ], [ -4769, 4355 ],
  [ -4769, 4532 ], [ -5100, 4532 ], [ -5100, 4355 ],
  [ -4683, 3816 ], [ -4216, 3653 ], [ -4069, 3959 ],
  [ -3934, 4323 ], [ -3913, 4750 ], [ -3913, 4929 ],
  [ -3913, 4983 ], [ -3913, 5056 ], [ -4022, 5137 ],
  [ -4081, 4983 ], [ -4081, 4944 ], [ -4081, 4929 ],
  [ -4081, 4626 ], [ -3934, 4360 ], [ -3407, 3826 ],
  [ -3471, 3826 ], [ -3934, 4230 ], [ -4216, 4374 ],
  [ -4260, 4421 ], [ -4493, 4655 ], [ -4875, 4532 ],
  [ -4875, 4355 ], [ -4598, 4355 ], [ -4598, 4532 ],
  [ -4607, 5279 ], [ -5159, 5871 ], [ -5159, 5959 ],
  [ -4821, 6380 ]
]
console.log(commonPoints.map(val => manhattenDistance([0, 0], val)));
